...
class Base(models.AbstractModel):
    ...
    def get_gantt_data(
        ...
        all_records = self.search_fetch(
            [("id", "in", all_record_ids)], read_specification.keys()
        )
        ...
        for group in final_result["groups"]:
            ...
            group["__record_ids"] = list(
                ordered_set_ids & OrderedSet(group["__record_ids"])
            )
    ...
    def web_gantt_reschedule(
        ...
        if direction not in (
            ...
            self._WEB_GANTT_RESCHEDULE_BACKWARD,
        ):
            raise ValueError("Invalid direction %r" % direction)

        master_record, slave_record = self.env[self._name].browse(
            [master_record_id, slave_record_id]
        )

        search_domain = [
            (dependency_field_name, "in", master_record.id),
            ("id", "=", slave_record.id),
        ]
        if not self.env[self._name].search(search_domain, limit=1):
            raise ValueError(
                "Record '%r' is not a parent record of '%r'"
        ...
        if not self._web_web_gantt_reschedule_is_relation_candidate(
            ...
            return {
                ...
                "params": {
                    "type": "warning",
                    "message": _(
                        "You cannot reschedule %s towards %s.",
                        master_record.name,
                        slave_record.name,
                    ),
                },
        ...
        is_master_prior_to_slave = (
            master_record[stop_date_field_name] <= slave_record[start_date_field_name]
        )
        ...
        if is_master_prior_to_slave ^ (
            direction == self._WEB_GANTT_RESCHEDULE_BACKWARD
        ):
        ...
        result = (
            result is True
            and trigger_record._web_gantt_action_reschedule_related_records(
                dependency_field_name,
                dependency_inverted_field_name,
                start_date_field_name,
                stop_date_field_name,
        ...
        if result is not True:
            if result is False:
                ...
                message = _(
                    "Records that are in the past cannot be automatically rescheduled. They should be manually rescheduled instead."
                )
    ...
    def _web_gantt_action_reschedule_related_records(
        ...
        for rescheduling_candidate in rescheduling_candidates:
            ...
            if record_write_result:
                ...
                record_ids_to_exclude[record.id] = record_ids_to_exclude[
                    related_record.id
                ] + [related_record.id]
        ...
        related_records_result = (
            records_to_propagate._web_gantt_action_reschedule_related_records(
                dependency_field_name,
                dependency_inverted_field_name,
                start_date_field_name,
                stop_date_field_name,
    ...
    def _web_gantt_get_rescheduling_candidates(
        ...
        for record in self:
            if record.id in record_ids_to_exclude[
                record.id
            ] or not record._web_gantt_reschedule_is_record_candidate(
            ...
            for master_record in record[dependency_field_name]:
                ...
                if (
                    master_record.id in record_ids_to_exclude[record.id]
            ...
            for slave_record in record[dependency_inverted_field_name]:
                ...
                if (
                    slave_record.id in record_ids_to_exclude[record.id]
        ...
        # with an even number of members.
        if set.intersection(
            set(slave_per_record.keys()), set(master_per_record.keys())
        ):
            if set.intersection(
                *map(set, [record_ids_to_exclude[rec.id] for rec in self])
            ):
                return False
        ...
        if any(record in slave_per_record.keys() for record in self) and any(
            record in master_per_record.keys() for record in self
        ):
            return False
        ...
        return [
            (
                ...
                slave_per_record[record_to_reschedule]
                or master_per_record[record_to_reschedule],
    ...
    @api.model
    def _web_gantt_reschedule_is_in_conflict(
        self, master, slave, start_date_field_name, stop_date_field_name
    ):
    ...
    @api.model
    def _web_web_gantt_reschedule_is_relation_candidate(
        self, master, slave, start_date_field_name, stop_date_field_name
    ):
    ...
    def _web_gantt_reschedule_record(
        ...
        if search_forward:
            date_candidate = related_record[stop_date_field_name].replace(
                tzinfo=timezone.utc
            )
        else:
            date_candidate = related_record[start_date_field_name].replace(
                tzinfo=timezone.utc
            )
    ...
    def _web_gantt_reschedule_write_new_dates(
        ...
        self.write(
            {
                start_date_field_name: new_start_date.astimezone(timezone.utc).replace(
                    tzinfo=None
                ),
                stop_date_field_name: new_stop_date.astimezone(timezone.utc).replace(
                    tzinfo=None
                ),
            }
        )
