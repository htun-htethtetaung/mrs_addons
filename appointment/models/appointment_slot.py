...
class AppointmentSlot(models.Model):
    ...
    slot_type = fields.Selection(
        [("recurring", "Recurring"), ("unique", "One Shot")],
        string="Slot type",
        default="recurring",
        required=True,
    ...
    def _check_delta_hours(self):
        if any(
            self.filtered(
                lambda slot: slot.start_hour
                + slot.appointment_type_id.appointment_duration
        ...
        for slot in self:
            if slot.slot_type == "recurring":
                slot.display_name = "%s, %02d:%02d - %02d:%02d" % (
                    ...
                    int(slot.start_hour),
                    int(round((slot.start_hour % 1) * 60)),
                    int(slot.end_hour),
                    int(round((slot.end_hour % 1) * 60)),
                )
            else:
