...
class CalendarEvent(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if (
            ...
            res["stop"] = datetime.min + round(
                (res["stop"] - datetime.min) / timedelta(minutes=1)
            ) * timedelta(minutes=1)
        ...
        if self.env.context.get("appointment_default_assign_user_attendees"):
            ...
            # Special gantt case: we want to assign the current user to the attendees if he's set as organizer
                ...
                and "partner_ids" in fields_list
            ):
    ...
    access_token = fields.Char(
        "Access Token", default=_default_access_token, readonly=True
    )
    alarm_ids = fields.Many2many(
        compute="_compute_alarm_ids", store=True, readonly=False
    )
    appointment_answer_input_ids = fields.One2many(
        "appointment.answer.input", "calendar_event_id", string="Appointment Answers"
    )
    appointment_attended = fields.Boolean("Attendees Arrived")
    appointment_type_id = fields.Many2one(
        "appointment.type", "Appointment", tracking=True
    )
    ...
    appointment_type_manage_capacity = fields.Boolean(
        related="appointment_type_id.resource_manage_capacity"
    )
    ...
    appointment_resource_id = fields.Many2one(
        ...
        inverse="_inverse_appointment_resource_id_or_capacity",
        store=True,
    ...
    booking_line_ids = fields.One2many(
        "appointment.booking.line", "calendar_event_id", string="Booking Lines"
    )
    partner_ids = fields.Many2many(
        "res.partner", group_expand="_read_group_partner_ids"
    )
    resource_total_capacity_reserved = fields.Integer(
        "Total Capacity Reserved",
        compute="_compute_resource_total_capacity",
        inverse="_inverse_appointment_resource_id_or_capacity",
    )
    resource_total_capacity_used = fields.Integer(
        "Total Capacity Used", compute="_compute_resource_total_capacity"
    )
    ...
    resources_on_leave = fields.Many2many(
        "appointment.resource",
        string="Resources intersecting with leave time",
        compute="_compute_resources_on_leave",
    )
    _sql_constraints = [
        (
    ...
    def _compute_appointment_resource_id(self):
        for event in self:
            if len(event.booking_line_ids) == 1:
                event.appointment_resource_id = (
                    event.booking_line_ids.appointment_resource_id
                )
            else:
    ...
    def _compute_resources_on_leave(self):
        resource_events = self.filtered(
            lambda event: event.appointment_resource_ids
            or event.appointment_resource_id
        )
        ...
        for start, stop, events in interval_from_events(resource_events):
            group_resources = (
                events.appointment_resource_ids | events.appointment_resource_id
            )
            unavailabilities = (
                group_resources.sudo().resource_id._get_unavailable_intervals(
                    start, stop
                )
            )
            for event in events:
                event_resources = (
                    event.appointment_resource_ids | event.appointment_resource_id
                )
                event.resources_on_leave = event_resources.filtered(
                    lambda resource: any(
                        intervals_overlap(interval, (event.start, event.stop))
                        for interval in unavailabilities.get(
    ...
    def _compute_resource_total_capacity(self):
        ...
        for event in self:
            ...
            event.resource_total_capacity_reserved = (
                data.get("total_capacity_reserved", 0) if data else 0
            )
            event.resource_total_capacity_used = (
    ...
    def _compute_videocall_redirection(self):
        for event in self:
            ...
            event.videocall_redirection = (
                f"{self.get_base_url()}/calendar/videocall/{self.access_token}"
            )
    ...
    def _compute_videocall_source(self):
        ...
        for event in self:
            if (
                not event.appointment_type_id
                or event.videocall_location
                and not self.DISCUSS_ROUTE in event.videocall_location
            ):
        ...
        for event in self:
            if not event.booking_line_ids and event.appointment_resource_id:
                ...
                event.booking_line_ids.capacity_reserved = min(
                    event.resource_total_capacity_reserved
                    or event.booking_line_ids.capacity_reserved,
    ...
    def read_group(
        ...
        groupby = [
            (
                group_element
                if group_element != "resource_ids"
                else "appointment_resource_ids"
            )
            for group_element in groupby
        ]
        read_group_data = super().read_group(
    ...
    def action_cancel_meeting(self, partner_ids):
        ...
        if attendees:
            ...
            message_body = _(
                "Appointment canceled by: %(partners)s", partners=cancelling_attendees
            )
            if self.appointment_booker_id.id == partner_ids[0]:
                ...
                self.message_post(
                    body=message_body, message_type="notification", author_id=author_id
                )
    ...
    def _find_or_create_partners(self, guest_emails_str):
        ...
        emails_normalized = [
            email_normalize(email, strict=False) for _, email in name_emails
        ]
        ...
        if "appointment_type_id" in changes:
            try:
                booked_template = self.env.ref(
                    "appointment.appointment_booked_mail_template"
                )
                    ...
                    booked_template.sudo(),
                    {
                        **vals,
                        ...
                        "subtype_id": self.env["ir.model.data"]._xmlid_to_res_id(
                            "appointment.mt_calendar_event_booked"
                        ),
            ...
            res["active"] = (
                appointment_type_sudo.canceled_mail_template_id,
                {
                    **vals,
                    "auto_delete_keep_log": False,
                    "subtype_id": self.env["ir.model.data"]._xmlid_to_res_id(
                        "appointment.mt_calendar_event_canceled"
                    ),
                    "email_layout_xmlid": "mail.mail_notification_light",
                },
            )
    ...
    def _get_customer_description(self):
        ...
        if self.appointment_type_id:
            ...
            if self.partner_id and (
                self.partner_id.name or self.partner_id.email or self.partner_id.phone
            ):
                email_detail_line = (
                    _("Email: %(email)s", email=self.partner_id.email)
                    if self.partner_id.email
                    else ""
                )
                phone_detail_line = (
                    _("Phone: %(phone)s", phone=self.partner_id.phone)
            ...
            return (
                f"{html2plaintext(message_confirmation)}\n\n{contact_details}".strip()
            )
    ...
    def gantt_unavailability(
        ...
        resource_ids = [
            row["resId"] for row in rows if row.get("resId")
        ]  # remove empty rows
        if (
            not group_bys
            or group_bys[0] not in ("resource_ids", "partner_ids")
            or not resource_ids
        ):
            return super().gantt_unavailability(
                start_date, end_date, scale, group_bys=group_bys, rows=rows
            )
        ...
        if appointment_type:
            slot_available_intervals = [
                ...
                for slot in appointment_type._slots_generate(
                    ...
                    end_datetime_utc,
                    "utc",
                    reference_date=start_datetime_utc,
                )
            ]
            slots_unavailable_intervals = invert_intervals(
                slot_available_intervals, start_datetime_utc, end_datetime_utc
            )

        # in staff view, add conflicting events to unavailabilities and return
        if group_bys[0] == "partner_ids":
            unavailabilities = self._gantt_unavailabilities_events(
                start_datetime,
                end_datetime,
                self.env["res.partner"].browse(resource_ids),
            )
            for row in rows:
                row_unavailabilities = unavailabilities.get(row["resId"], Intervals([]))
                row_unavailabilities |= Intervals(
                    [
                        (start, stop, self.env["res.partner"])
                        for start, stop in slots_unavailable_intervals
                    ]
                )
                row["unavailabilities"] = [
                    {"start": start, "stop": stop}
                    for start, stop, _ in row_unavailabilities
                ]
            return rows
        ...
        resource_unavailabilities = (
            appointment_resource_ids.resource_id._get_unavailable_intervals(
        ...
        for row in rows:
            ...
            unavailabilities = Intervals(
                [
                    (start, stop, set())
                    for start, stop in resource_unavailabilities.get(
                        appointment_resource_id.resource_id.id, []
                    )
                ]
            )
            unavailabilities |= Intervals(
                [(start, stop, set()) for start, stop in slots_unavailable_intervals]
            )
            row["unavailabilities"] = [
                {"start": start, "stop": stop} for start, stop, _ in unavailabilities
            ]
        return rows
        ...
        return {
            attendee.id: Intervals(
                [
                    ...
                    for event in partners._get_calendar_events(
                        start_datetime, end_datetime
                    ).get(attendee.id, [])
                ]
            )
    ...
    def get_gantt_data(self, domain, groupby, read_specification, limit=None, offset=0):
        ...
        if (
            self.env.context.get("appointment_booking_gantt_show_all_resources")
            and groupby
            and groupby[0] == "partner_ids"
        ):
            staff_partner_ids = (
                self.env["appointment.type"]
                .search([("schedule_based_on", "=", "users")])
                .staff_user_ids.partner_id.ids
            )
            gantt_data["groups"] = [
                group
                for group in gantt_data["groups"]
                if group.get("partner_ids")
                and group["partner_ids"][0] in staff_partner_ids
            ]
        return gantt_data
