...
class ResourceCalendar(models.Model):
    ...
    def _unavailable_intervals_batch(
        ...
        intervals = super()._unavailable_intervals_batch(
            start_dt, end_dt, resources, domain, tz
        )
        resource_ids = (
            self.env["appointment.resource"].sudo().search([]).resource_id.ids
        )
