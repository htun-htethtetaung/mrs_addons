...
class CalendarAppointmentAnswer(models.Model):
    ...
    question_id = fields.Many2one(
        "appointment.question", "Question", required=True, ondelete="cascade"
    )
...
class CalendarAppointmentAnswerInput(models.Model):
    ...
    value_answer_id = fields.Many2one(
        "appointment.answer", "Selected Answer", ondelete="restrict"
    )
    value_text_box = fields.Text("Text Answer")
    # Reporting
    appointment_type_id = fields.Many2one(
        related="question_id.appointment_type_id",
        required=True,
        store=True,
        ondelete="cascade",
    )
    calendar_event_id = fields.Many2one(
        "calendar.event", "Calendar Event", required=True, ondelete="cascade"
    )
