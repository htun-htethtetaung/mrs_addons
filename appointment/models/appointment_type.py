...
class AppointmentType(models.Model):
    ...
    def _default_booked_mail_template_id(self):
        return self.env["ir.model.data"]._xmlid_to_res_id(
            "appointment.attendee_invitation_mail_template"
        )

    def _default_canceled_mail_template_id(self):
        return self.env["ir.model.data"]._xmlid_to_res_id(
            "appointment.appointment_canceled_mail_template"
        )
    ...
    user_assign_method = fields.Selection(
        ...
        compute="_compute_user_assign_method",
        inverse="_inverse_user_assign_method",
        help="How users and resources will be assigned to meetings customers book on your website.",
    )
    avatars_display = fields.Selection(
        [("hide", "No Picture"), ("show", "Show Pictures")],
        string="Front-End Display",
        compute="_compute_avatars_display",
        readonly=False,
        store=True,
    ...
    question_ids = fields.One2many(
        "appointment.question", "appointment_type_id", string="Questions", copy=True
    )
    ...
    staff_user_count = fields.Integer(
        "# Staff Users", compute="_compute_staff_user_count"
    )
    ...
    resource_manual_confirmation = fields.Boolean(
        "Manual Confirmation",
        compute="_compute_resource_manual_confirmation",
        store=True,
        readonly=False,
    ...
    resource_manage_capacity = fields.Boolean(
        "Manage Capacities",
    ...
    appointment_count = fields.Integer(
        "# Appointments", compute="_compute_appointment_count"
    )
    appointment_count_report = fields.Integer(
        "# Appointments in the last 30 days",
        compute="_compute_appointment_count_report",
    )
    appointment_invite_ids = fields.Many2many(
        "appointment.invite", string="Invitation Links"
    )
    ...
    def _compute_appointment_count(self):
        ...
        mapped_data = {
            appointment_type.id: count for appointment_type, count in meeting_data
        }
    ...
    def _compute_appointment_count_report(self, n_days=30):
        from_n_days_ago = datetime.combine(
            datetime.today().date() - timedelta(days=n_days), datetime.min.time()
        )
        until_yesterday = datetime.combine(datetime.today().date(), datetime.max.time())
        meeting_data = self.env["calendar.event"]._read_group(
            [
                ("appointment_type_id", "in", self.ids),
                ("start", ">=", from_n_days_ago),
                ("start", "<=", until_yesterday),
            ],
            ["appointment_type_id"],
            ["__count"],
        )
        ...
        for appointment_type in self:
            appointment_type.appointment_count_report = mapped_data.get(
    ...
    def _compute_appointment_duration_formatted(self):
        for record in self:
            ...
            ].value_to_html(record.appointment_duration * 3600, {})
    ...
    def _compute_category(self):
        for appointment_type in self:
            appointment_type.category = (
                "punctual" if appointment_type.end_datetime else "recurring"
            )
    ...
    def _check_appointment_category_time_boundaries(self):
        for appointment_type in self:
            ...
            elif appointment_type.category != "punctual" and (
    ...
    def _check_staff_user_configuration(self):
        ...
        for appointment_type in self.filtered(
            ...
            if (
                appointment_type.category == "anytime"
                and len(appointment_type.staff_user_ids) != 1
            ):
                raise ValidationError(
                    _(
                        "Anytime appointment types should only have one user but got %s users",
                        len(appointment_type.staff_user_ids),
                    )
                )
            invalid_restricted_users = (
                appointment_type.slot_ids.restrict_to_user_ids
                - appointment_type.staff_user_ids
            )
            if invalid_restricted_users:
                raise ValidationError(
                    _(
                        "The following users are in restricted slots but they are not part of the available staff: %s",
                        ", ".join(invalid_restricted_users.mapped("name")),
                    )
                )
            if appointment_type.category == "anytime":
                duplicate = anytime_appointments.filtered(
                    lambda apt_type: bool(
                        apt_type.staff_user_ids & appointment_type.staff_user_ids
                    )
                )
                if appointment_type.ids:
                    duplicate = duplicate.filtered(
                        lambda apt_type: apt_type.id not in appointment_type.ids
                    )
                if duplicate:
                    raise ValidationError(
                        _(
    ...
    def action_appointment_resources(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "appointment.appointment_resource_action"
        )
    ...
    def action_appointment_shared_links(self):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "appointment.appointment_invite_action"
        )
        action["domain"] = [("appointment_type_ids", "in", self.ids)]
        if len(self) == 1:
    ...
    def action_calendar_events_reporting(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "appointment.calendar_event_action_appointment_reporting"
        )
    ...
    def action_calendar_meetings(self):
        ...
        action = AppointmentType.insert_reorder_action_views(
            action, management_views + ["calendar", "pivot"]
        )
        ...
        action["context"].update(
            {
                ...
                "initial_date": (
                    appointments[0].start if appointments else datetime.today()
                ),
            }
        )
        return action
    ...
    def _get_gantt_scale(self):
        ...
        last_meeting_end = (
            last_meeting_values[0]["stop"] if last_meeting_values else False
        )
    ...
    def _get_default_appointment_attendee_status(
        self, start_dt, stop_dt, capacity_reserved
    ):
        ...
        if self.schedule_based_on == "resources" and self.resource_manual_confirmation:
            ...
            if (
                float_compare(
                    resource_total_capacity_used / self.resource_total_capacity,
                    self.resource_manual_confirmation_percentage,
                    2,
                )
                >= 0
            ):
    ...
    def _slots_generate(self, first_day, last_day, timezone, reference_date=None):
        ...
        end_tz_apt_type = (
            self.end_datetime.astimezone(appt_tz)
        ...
        if self.category != "custom":
            ...
            for day in rrule.rrule(
                ...
                for slot in self.slot_ids.filtered(
                    lambda x: int(x.weekday) == day.isoweekday()
                ):
            ...
            unique_slots = self.slot_ids.filtered(
                lambda slot: slot.slot_type == "unique"
                and slot.end_datetime > reference_date
            )
    ...
    def _check_appointment_is_valid_slot(
        self, staff_user, resources, asked_capacity, timezone, start_dt, duration
    ):
        """
        ...
        if (
            ...
            self_sudo._slots_fill_users_availability(
                slots, start_dt, end_dt, staff_user
            )
        elif (
            slots
    ...
    def _prepare_calendar_event_values(
        ...
        attendee_status = self._get_default_appointment_attendee_status(
            start, stop, asked_capacity
        )
        attendee_values = [
            Command.create({"partner_id": pid, "state": attendee_status})
        ...
        return {
            ...
            "categ_ids": [
                Command.set(
                    appointment_invite._get_meeting_categories_for_appointment().ids
                )
            ],
    ...
    def _slots_fill_users_availability(
        self, slots, start_dt, end_dt, filter_users=None
    ):
            ...
            available_staff_user = next(
                (
                    ...
                    if self._slot_availability_is_user_available(
                        slot, staff_user, availability_values
                    )
                ),
                False,
            )
    ...
    def _slot_availability_is_user_available(
        self, slot, staff_user, availability_values
    ):
        ...
        if partner_to_events.get(staff_user.partner_id):
            for day_dt in rrule.rrule(
                ...
                if any(
                    not event.allday
                    and (
                        event.start < slot_end_dt_utc and event.stop > slot_start_dt_utc
                    )
    ...
    def _slot_availability_prepare_users_values(self, staff_users, start_dt, end_dt):
        ...
        """
        return self._slot_availability_prepare_users_values_meetings(
            staff_users, start_dt, end_dt
        )

    def _slot_availability_prepare_users_values_meetings(
        self, staff_users, start_dt, end_dt
    ):
        ...
        for event in all_events:
            for attendee in event.attendee_ids.filtered_domain(
                ...
                for day_dt in rrule.rrule(
                    ...
                    partner_events = partner_to_events.setdefault(
                        attendee.partner_id, {}
                    )
    ...
    def _slot_availability_is_resource_available(
        ...
        slot_start_dt_utc_l, slot_end_dt_utc_l = pytz.utc.localize(
            slot_start_dt_utc
    ...
    def _get_resources_remaining_capacity(
        ...
        if resource_to_bookings is None:
            ...
            booking_lines = booking_lines.filtered(
                lambda bl: bl.event_start < slot_stop_utc
                and bl.event_stop > slot_start_utc
            )
        ...
        resources_remaining_capacity = {
            ...
            - sum(
                booking_line.capacity_used
                for booking_line in resources_booking_lines.get(resource, [])
            )
            for resource in all_resources
        }
        resources_remaining_capacity.update(
            total_remaining_capacity=sum(resources_remaining_capacity.values())
        )
    ...
    def _slot_availability_prepare_resources_leave_values(
        ...
        """
        unavailabilities = (
            appointment_resources.sudo().resource_id._get_unavailable_intervals(
