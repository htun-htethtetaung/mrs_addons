...
class AppointmentBookingLine(models.Model):
    ...
    calendar_event_id = fields.Many2one(
        "calendar.event", string="Booking", ondelete="cascade", required=True
    )
    ...
    def _check_resources_match_appointment_type(self):
        ...
        for appointment_type, lines in self.grouped("appointment_type_id").items():
            non_compatible_resources = (
                lines.appointment_resource_id - appointment_type.resource_ids
            )
            if non_compatible_resources:
                raise ValidationError(
                    _(
                        '"%(resource_name_list)s" cannot be used for "%(appointment_type_name)s"',
                        appointment_type_name=appointment_type.name,
                        resource_name_list=", ".join(
                            non_compatible_resources.mapped("name")
                        ),
                    )
                )
    ...
    @api.depends(
        ...
        "appointment_type_id.resource_manage_capacity",
        "capacity_reserved",
    )
    def _compute_capacity_used(self):
        ...
        for line in self:
            if line.capacity_reserved == 0:
                ...
                or not line.appointment_type_id.resource_manage_capacity
            ):
