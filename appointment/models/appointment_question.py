...
class AppointmentQuestion(models.Model):
    ...
    appointment_type_id = fields.Many2one(
        "appointment.type", "Appointment Type", ondelete="cascade"
    )
    ...
    answer_ids = fields.One2many(
        "appointment.answer", "question_id", string="Available Answers", copy=True
    )
    answer_input_ids = fields.One2many(
        "appointment.answer.input", "question_id", string="Submitted Answers"
    )

    @api.constrains("question_type", "answer_ids")
    def _check_question_type(self):
        incomplete_questions = self.filtered(
            lambda question: question.question_type in ["select", "radio", "checkbox"]
            and not question.answer_ids
        )
