...
class AppointmentShare(models.Model):
    ...
    short_code = fields.Char(
        "Short Code", default=lambda s: s._get_unique_short_code(), required=True
    )
    ...
    appointment_type_ids = fields.Many2many(
        ...
        domain="[('category', 'in', ['punctual', 'recurring'])]",
    ...
    suggested_resource_count = fields.Integer(
        "# Resources", compute="_compute_suggested_resource_count"
    )
    ...
    suggested_staff_user_count = fields.Integer(
        "# Staff Users", compute="_compute_suggested_staff_user_count"
    )
    ...
    def _compute_appointment_type_info_msg(self):
        ...
        for invite in self:
            ...
            invite.appointment_type_info_msg = (
                appointment_type_info_msg if appointment_type_info_msg else False
            )
    ...
    def _compute_short_code_warning(self):
        for invite in self:
            ...
            invite.short_code_unique_warning = bool(
    ...
    def _get_unique_short_code(self, short_code=False):
        short_access_token = (
            self.access_token[:8] if self.access_token else uuid.uuid4().hex[:8]
        )
        short_code = short_code or self.short_code or short_access_token
        nb_short_code = self.env["appointment.invite"].search_count(
            [("id", "!=", self._origin.id), ("short_code", "=", short_code)]
        )
    ...
    def _gc_appointment_invite(self):
        ...
        to_remove = invites.filtered(
            lambda invite: not invite.calendar_event_ids
