...
class AppointmentResource(AppointmentCommon):
    ...
    def test_appointment_resource_field(self):
        ...
        self.assertEqual(
            booking.booking_line_ids.appointment_resource_id, self.resource_1
        )
        ...
        self.assertFalse(
            booking.appointment_resource_id,
            "More than one booking lines should mean no singular resource id.",
        )
        self.assertEqual(
            booking.booking_line_ids.appointment_resource_id,
        ...
        self.assertEqual(
            len(booking.booking_line_ids),
            2,
            "Setting the singular resource when there already are multiple booking lines should do nothing.",
        )
    ...
    def test_appointment_resources_availability(self):
        ...
        with freeze_time(self.reference_now):
            slots = appointment._get_appointment_slots(
                timezone="UTC", filter_resources=resource
            )
        ...
        self.assertSlots(
            ...
            ],
            {
                ...
                "slots_weekdays_nowork": range(
                    1, 7
                ),  # Resource only available on monday (0)
            },
        )
