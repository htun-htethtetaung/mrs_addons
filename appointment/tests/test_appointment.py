...
class AppointmentTest(AppointmentCommon, HttpCaseWithUserDemo):
    ...
    def test_appointment_availability_with_show_as(self):
        ...
        self.assertTrue(
            apt_types[-1]._check_appointment_is_valid_slot(
                ...
                "UTC",
                datetime(
                    2023, 1, 9, 8, 0, tzinfo=timezone.utc
        ...
        for unique_slot, apt_type, is_available in zip(
            unique_slots, apt_types, [True, False]
        ):
            duration = (
            ...
            self.assertEqual(
                apt_type._check_appointment_is_valid_slot(
                    employee, 0, 0, "UTC", unique_slot["start_datetime"], duration
                ),
                is_available,
            )
    ...
    def test_appointment_type_custom_badge(self):
        ...
        nb_february_slots = len(
            self._filter_appointment_slots(
                slots, filter_months=[(2, 2022)], filter_users=self.apt_manager
            )
        )
        nb_march_slots = len(
            self._filter_appointment_slots(
                slots, filter_months=[(3, 2022)], filter_users=self.apt_manager
            )
        )
        nb_april_slots = len(
            self._filter_appointment_slots(
                slots, filter_months=[(4, 2022)], filter_users=self.apt_manager
            )
        )
        ...
        self.assertEqual(
            slots[0]["nb_slots_next_months"], nb_march_slots + nb_april_slots
        )
        ...
        # April month
        self.assertEqual(
            slots[2]["nb_slots_previous_months"], nb_february_slots + nb_march_slots
        )
    ...
    def test_customer_event_description(self):
        ...
        appointment = self.env["calendar.event"].create(
            {
                ...
                "partner_ids": [
                    odoo.Command.link(partner.id)
                    for partner in [attendee, host_partner]
                ],
        ...
        for changes in [
            ...
            self.assertEqual(
                appointment._get_customer_description(), (message + details).strip()
            )
            self.assertEqual(
                appointment._get_customer_summary(),
    ...
    def test_generate_slots_punctual_appointment_type(self):
        ...
        for (start_datetime, end_datetime), (
            ...
            with self.subTest(start_datetime=start_datetime, end_datetime=end_datetime):
                ...
                reference_date = (
                    start_datetime
                    if start_datetime > self.reference_now
                    else self.reference_now
                )
    ...
    def test_generate_slots_recurring_start_hour_day_overflow(self):
        ...
        for hour, minute in [[h, m] for h in [2, 9, 10, 11, 12] for m in [0, 1, 59]]:
            time = brussels_tz.localize(
                self.reference_monday.replace(hour=hour, minute=minute)
            )
            ...
            self.assertSlots(
                ...
                ],
                {
                    ...
                    "slots_day_specific": {  # +4 instead of +4.5 because the test method only accounts for the absolute hour
                        time.date(): (
                            [{"start": 15 + 4, "end": 16 + 4}] if hour == 2 else []
                        ),  # min_schedule_hours is too large
    ...
    def test_slots_for_today(self):
        ...
        for slot in slots:
            self.assertTrue(
                test_reference_now.astimezone(pytz.UTC)
                < slot["UTC"][0].astimezone(pytz.UTC),
