...
class AppointmentUITest(AppointmentUICommon):
    ...
    def test_action_meeting_from_appointment_type(self):
        ...
        expected_contexts = [
            ...
            {
                "appointment_booking_gantt_domain": [
        ...
        for appointment_type, expected_views_order, expected_context, xml_id in zip(
            ...
            with self.subTest(appointment_type=appointment_type):
                ...
                views_order = [
                    view_id_type[1]
                    for view_id_type in action["views"][: len(expected_views_order)]
                ]
    ...
    def test_appointment_meeting_url(self):
        ...
        for appointment, appointment_data, expect_discuss in cases:
            ...
            new_appt_datetime = pytz.timezone(appointment.appointment_tz).localize(
                appt_datetime
            ) + timedelta(hours=1)
            ...
            event = CalendarEvent.search(
                [
                    ...
                    ("start", "=", new_appt_datetime.astimezone(pytz.utc)),
                ]
            )
    ...
    def test_get_appointment_type_page_view(self):
        ...
        self._create_meetings(
            self.user_employee, [(slot_time, slot_time + timedelta(hours=1), True)]
        )
        ...
        self._create_meetings(
            self.user_admin, [(slot_time, slot_time + timedelta(hours=1), True)]
        )
...
class CalendarTest(AppointmentUICommon):
    ...
    def test_meeting_cancel_authenticated(self):
        ...
        expected_attendee = (
            self.staff_user_aust.partner_id + self.staff_user_nz.partner_id
        )
