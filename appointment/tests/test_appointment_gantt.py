        ...
        cls.apt_types = cls.env["appointment.type"].create(
            [
                ...
                },
                {
                    ...
                    "resource_ids": [
                        (4, cls.apt_resource_1.id),
                        (4, cls.apt_resource_2.id),
                    ],
...
class AppointmentGanttTest(AppointmentGanttTestCommon):
    ...
    def test_default_assign_user_attendees(self):
        ...
        event_with_partner = (
            ...
            .with_context(single_attendee_context)
            .create(
                {
        ...
        event_without_partner = (
            self.env["calendar.event"]
            .with_context(no_attendees_context)
            .create(
                {
    ...
    def test_gantt_empty_groups(self):
        ...
        context = self.gantt_context | {
            "default_appointment_type_id": self.apt_types[0].id
        }
        gantt_data = (
            self.env["calendar.event"]
        ...
        self.assertNotIn(
            self.user_john.partner_id.id,
            group_partner_ids,
            "Should only display staff assigned to the default apt type.",
        )
    ...
    def test_gantt_hide_non_staff(self):
        ...
        meeting = self._create_meetings(
            ...
            [
                (
                    ...
                    self.reference_monday + timedelta(hours=1),
                    False,
                )
            ],
            self.apt_types[0].id,
        ...
        self.assertNotIn(
            self.partners[0].id,
            group_partner_ids,
            "Attendees with no users should be hidden from the grouping.",
        )
        self.assertIn(self.user_bob.partner_id.id, group_partner_ids)
    ...
    def test_gantt_resource_unavailabilities_multi_company(self):
        ...
        resource_calendar_unavailabilities = Intervals(
            [
                (
                    datetime(2022, 2, 14, 11, 00, tzinfo=pytz.UTC),
                    datetime(2022, 2, 14, 11, 0, tzinfo=pytz.UTC),
                    set(),
                ),
            ]
        )
        ...
        for (
            ...
            res_1_company,
            res_2_company,
            res_1_unavailabilities,
            res_2_unavailabilities,
        ) in [
            (
                [],
                False,
                False,
                resource_calendar_unavailabilities,
                resource_calendar_unavailabilities,
            ),
            (
                [],
            ...
            with self.subTest(
                ...
                self.assertListEqual(
                    list(
                        Intervals(
                            [
                                (unavailability["start"], unavailability["stop"], set())
                                for unavailability in unavailabilities[0][
                                    "unavailabilities"
                                ]
                            ]
                        )
                ...
                self.assertListEqual(
                    list(
                        Intervals(
                            [
                                (unavailability["start"], unavailability["stop"], set())
                                for unavailability in unavailabilities[1][
                                    "unavailabilities"
                                ]
                            ]
                        )
