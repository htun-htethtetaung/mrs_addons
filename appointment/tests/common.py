...
class AppointmentCommon(MailCommon, common.HttpCase):
    ...
    def mockAppointmentCalls(self):
        ...
        with patch.object(
            Meeting, "search", autospec=True, side_effect=_original_search
        ) as mock_ce_search, patch.object(
            Meeting, "search_count", autospec=True, side_effect=_original_search_count
        ) as mock_ce_sc, patch.object(
            Partner,
            "calendar_verify_availability",
            autospec=True,
            side_effect=_original_calendar_verify_availability,
        ) as mock_partner_cal, patch.object(
            ResourceCalendar,
