...
class AppointmentTestTracking(AppointmentCommon, MailCase):
            ...
            .create(
                {
                    ...
                    "partner_ids": [
                        Command.link(p)
                        for p in cls.apt_manager.partner_id.ids
                        + cls.appointment_attendee_ids.ids
                    ],
    ...
    def test_archive_message(self):
        ...
        self.assertGreater(
            meeting.start,
            datetime.now(),
            "Test expects `datetime.now` to be before start of meeting",
        )
        ...
        self.assertEqual(
            len(self._new_msgs),
            2,
            "Expected a tracking message and a cancelation template",
        )
        self.assertTracking(self._new_msgs[0], [("active", "boolean", True, False)])
        self.assertEqual(
            self.ref("appointment.mt_calendar_event_canceled"),
        ...
        self.assertTracking(self._new_msgs[0], [("active", "boolean", False, True)])
    ...
    def test_cancel_meeting_message(self):
        ...
        self.assertEqual(
            meeting.partner_ids,
            self.apt_manager.partner_id + self.appointment_attendee_ids,
            "Manager and attendees should be there",
        )
        self.assertEqual(
            meeting.message_partner_ids,
            permanent_followers + self.appointment_attendee_ids,
            "All attendees and concerned users should be followers",
        )

        with self.mock_mail_gateway(), self.mock_mail_app():
            meeting.with_context(mail_notify_force_send=True).action_cancel_meeting(
                self.appointment_attendee_ids[0].ids
            )
            self.flush_tracking()

        self.assertEqual(
            meeting.partner_ids,
            self.apt_manager.partner_id + self.appointment_attendee_ids[1],
        ...
        self.assertEqual(
            len(self._new_msgs), 1, "Should be a single message for the cancelation"
        )
        ...
        with self.mock_mail_gateway(), self.mock_mail_app():
            meeting.with_user(self.apt_manager).action_cancel_meeting(
                self.appointment_attendee_ids[1].ids
            )
            self.flush_tracking()
        ...
        self.assertEqual(
            ...
            "Only the manager should remain",
        )
        self.assertEqual(
            meeting.message_partner_ids,
            permanent_followers,
        ...
        self.assertEqual(
            len(self._new_msgs), 1, "Should be a message saying who canceled"
        )
        self.assertMessageFields(
            self._new_msgs[0],
            {
