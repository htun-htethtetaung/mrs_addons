...
class AppointmentManageLeaveTest(AppointmentCommon):
    ...
    def test_manage_leaves(self):
        ...
        for slot_resources in [
            resource["available_resources"]
            for resource in self._filter_appointment_slots(slots)
        ]:
            self.assertEqual(
                set(slot_resource["id"] for slot_resource in slot_resources),
                set(resources.ids),
            )
        ...
        available_resources_14h = [
            resource["id"]
            for resource in self._filter_appointment_slots(slots)[14 - 9][
                "available_resources"
            ]
        ]
        available_resources_15h = [
            resource["id"]
            for resource in self._filter_appointment_slots(slots)[15 - 9][
                "available_resources"
            ]
        ]
        available_resources_16h = [
            resource["id"]
            for resource in self._filter_appointment_slots(slots)[16 - 9][
                "available_resources"
            ]
        ]
        self.assertEqual(set(available_resources_14h), set(resources.ids))
        self.assertEqual(
            set(available_resources_15h),
            set((resources - resources[0]).ids),
            "Resource should be on leave and excluded from slots",
        )
        ...
        self.assertSlots(
            ...
            ],
            {
                "enddate": global_slots_enddate,
                "startdate": global_slots_startdate,
                "slots_start_hours": [hour for hour in range(9, 15)]
                + [hour for hour in range(16, 17)],
                "slots_startdate": start_monday.date(),  # first Monday after reference_now
                "slots_enddate": start_monday.date(),  # only test that day
            },
        )
        ...
        with freeze_time(start_sunday):
            slots = appt_type._get_appointment_slots("Europe/Brussels")
        self.assertEqual(
            len(self._filter_appointment_slots(slots)),
            9,
            "Leaves, bookings and work hours should be taken into account on last day",
        )
