    ...
    def _populate_factories(self):
        def compute_hours_and_duration(iterator, *args):
            ...
            for values in iterator:
                app_type = self.env["appointment.type"].browse(
                    values["appointment_type_id"]
                )
                app_type_long_duration = (
                    app_type.appointment_duration
                    > appointment_type_duration_max_half_day
                )
                ...
                # be tagged 'allday')
                if random.random() < 0.2 or app_type_long_duration:
                    yield {
                        **values,
                        "start_hour": random.choices(
                            *zip(*data.appointment_slot.get("start_time_am").items())
                        )[0],
                        "end_hour": random.choices(
                            *zip(*data.appointment_slot.get("end_time_pm").items())
                        )[0],
                        "allday": (
                            random.random() < 0.5 if app_type_long_duration else False
                        ),
                    }
                else:  # Yield a morning and an afternoon slot
                    for start_hours, end_hours in zip(
                        ["start_time_am", "start_time_pm"],
                        ["end_time_am", "end_time_pm"],
                    ):
                        yield {
                            **values,
                            "start_hour": random.choices(
                                *zip(*data.appointment_slot.get(start_hours).items())
                            )[0],
                            "end_hour": random.choices(
                                *zip(*data.appointment_slot.get(end_hours).items())
                            )[0],
                            "allday": False,
                        }
            ...
            for values in iterator:
                ...
                if app_type.category != "custom":
                    ...
                    new_values = {
                        ...
                        + datetime.timedelta(hours=app_type["appointment_duration"]),
                        "allday": False,
                    }
        ...
        appointment_type_ids = (
            ...
            .browse(self.env.registry.populated_models["appointment.type"])
            .filtered_domain([("category", "in", ["punctual", "recurring", "custom"])])
            .ids
        )
