...
class AppointmentType(models.Model):
    ...
    def _populate_factories(self):
        ...
        company_users = {
            company_id: (company_id.user_ids - admin_ids).ids
            for company_id in company_ids
            if len(company_id.user_ids - admin_ids) > 0
        }
        ...
        def get_tz(values, random=None, **kwargs):
            ...
            return random.choice(
                self.env["res.users"].browse(values["staff_user_ids"])
            ).tz

        def get_up_to_two_reminders(random=None, **kwargs):
            reminders = self.env["calendar.alarm"].browse(
                random.sample(reminder_ids, 2)
            )
            # Avoid sending two reminders at same time with same method
            if (
                len(reminders.mapped("alarm_type")) == 1
        ...
        def get_n_countries_or_false(random=None, **kwargs):
            n_countries = random.choices(
                (0, 1, 4, 5, 8), (0.80, 0.05, 0.05, 0.05, 0.05)
            )[0]
            if n_countries:
        ...
        return [
            ...
            *(
                (
                    field_name,
                    populate.iterate(*zip(*data.appointment_type[field_name].items())),
                )
                for field_name in [
                    "min_schedule_hours",
                    "max_schedule_days",
            ...
            (
                ...
                populate.iterate(
                    *zip(
                        *data.appointment_type["appointment_duration_half_day"].items()
                    )
                ),
            ),
    ...
    def _populate(self, size):
        ...
        for record in appointment_types:
            ...
            if none_done_category or random.random() < 0.1 and record.staff_user_ids:
                single_staff_user = record.staff_user_ids[0]
                (
                    record.with_company(single_staff_user.company_id)
                    .with_user(single_staff_user.id)
                    .write(
                        {
            ...
            if none_done_duration or random.random() < 0.1:
                record.appointment_duration = random.choices(
                    *zip(*data.appointment_type["appointment_duration_all_day"].items())
                )[0]
