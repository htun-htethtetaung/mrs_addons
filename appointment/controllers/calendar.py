...
class AppointmentCalendarController(CalendarController):
    ...
    def view_meeting(self, token, id):
        ...
        attendee = (
            ...
            .sudo()
            .search([("access_token", "=", token), ("event_id", "=", int(id))])
        )
        if not attendee:
        ...
        return request.redirect(
            f"/calendar/view/{attendee.event_id.access_token}?partner_id={attendee.partner_id.id}"
        )
    ...
    def appointment_view(self, access_token, partner_id, state=False, **kwargs):
        ...
        if not timezone:
            timezone = (
                ...
                or event.partner_ids
                and event.partner_ids[0].tz
                or event.user_id.tz
                or "UTC"
            )
        ...
        if not event.allday:
            ...
            url_date_start = url_date_stop = fields.Date.from_string(
                event.start_date
            ).strftime("%Y%m%d")
        ...
        date_start = (
            f"{day_name} {format_func(date_start, locale=locale)}{date_start_suffix}"
        )
        params = {
            "action": "TEMPLATE",
    ...
    def appointment_add_attendee(self, access_token, emails_str):
        ...
        event_sudo = event_sudo.sudo().search(
    ...
    def appointment_cancel(self, access_token, partner_id, **kwargs):
        ...
        if fields.Datetime.from_string(
            ...
            return request.redirect(
                f"/calendar/view/{access_token}?state=no-cancel&partner_id={partner_id}"
            )
        event.with_context(mail_notify_author=True).sudo().action_cancel_meeting(
            [int(partner_id)]
        )
