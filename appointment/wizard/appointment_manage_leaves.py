...
class AppointmentManageLeaves(models.TransientModel):
    ...
    def _default_time(self, hour, minute):
        ...
        user_time = user_timezone.localize(
            fields.Datetime.today().replace(hour=hour, minute=minute)
        )
        return user_time.astimezone(pytz.utc).replace(tzinfo=None)

    appointment_resource_ids = fields.Many2many(
        "appointment.resource", string="Specific Resources"
    )
    ...
    def action_create_leave(self):
        ...
        for wizard in self:
            ...
            if wizard.appointment_resource_ids:
                ...
                leave_values += [
                    {"calendar_id": wizard.calendar_id.id, **leave_values_common}
                ]
