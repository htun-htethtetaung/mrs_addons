...
class AppointmentType(models.Model):
    ...
    appointment_type_warning_msg = fields.Char(
        "Different Website Message", compute="_compute_appointment_type_warning_msg"
    )
    ...
    def _compute_disable_save_button(self):
        ...
        for invite in self:
            invite.disable_save_button = invite.disable_save_button or bool(
                invite.appointment_type_warning_msg
            )
    ...
    def _compute_appointment_type_warning_msg(self):
        ...
        for invite in self.filtered("website_id"):
            appt_with_different_website = invite.appointment_type_ids.filtered_domain(
                [("website_id", "not in", [False, invite.website_id.id])]
            )
            if len(appt_with_different_website) > 0:
                invite.appointment_type_warning_msg = _(
                    "The following appointment type(s) are not compatible with the website chosen: "
                ) + ", ".join(appt_with_different_website.mapped("name"))
