...
class WebsiteAppointmentTest(AppointmentCommon, MockVisitor):
    ...
    def test_apt_type_create_from_website(self):
        ...
        with self.mock_visitor_from_request(force_visitor=visitor):
            ...
            AppointmentType.with_user(test_user).create_and_get_website_url(
    ...
    def test_apt_type_create_from_website_slots(self):
        ...
        expected_slots = {
            ...
            for start_hour, end_hour in ((9.0, 12.0), (14.0, 17.0))
        }
    ...
    def test_apt_type_is_published(self):
        for category, default in [
            ...
            appointment_type = self.env["appointment.type"].create(
                {
                    ...
                    "category": category,
                    "start_datetime": (
                        datetime(2023, 10, 3, 8, 0) if category == "punctual" else False
                    ),
                    "end_datetime": (
            ...
            if category in ["custom", "punctual", "recurring"]:
                ...
                self.assertFalse(
                    appointment_copied.is_published,
                    "When we copy an appointment type, the new one should not be published",
                )
    ...
    def test_apt_type_is_published_update(self):
        ...
        self.assertFalse(
            ...
            "A recurring appointment type should not be published at creation",
        )

        appointment.write({"category": "custom"})
        self.assertFalse(
            appointment.is_published,
            "Modifying an appointment type category should not modify the publish state",
        )
    ...
    def test_find_customer_country_from_visitor(self):
        ...
        with MockRequest(self.env, website=current_website) as mock_request:
            with self.mock_visitor_from_request(
                force_visitor=visitor_from_the_us
                ...
                available_appointments = (
                    wa_controller._fetch_and_check_private_appointment_types(
                        None,
                        None,
                        None,
                        "",
                        domain=wa_controller._appointments_base_domain(
                            None, False, None, domain
                        ),
                    )
                )

                self.assertNotIn(
                    appointments_belgium,
                    available_appointments,
