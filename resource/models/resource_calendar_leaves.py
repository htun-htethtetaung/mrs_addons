...
class ResourceCalendarLeaves(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if (
            "date_from" in fields_list
            and "date_to" in fields_list
            ...
            intervals = self.env.company.resource_calendar_id._work_intervals_batch(
                date_from.replace(tzinfo=utc), date_to.replace(tzinfo=utc)
            )[False]
            if intervals:  # Then we stop and return the dates given in parameter
                list_intervals = [
                    (start, stop) for start, stop, records in intervals
                ]  # Convert intervals in interval list
                date_from = list_intervals[0][
                    0
                ]  # We take the first date in the interval list
                date_to = list_intervals[-1][
                    1
                ]  # We take the last date in the interval list
            res.update(
    ...
    def _compute_date_to(self):
        ...
        for leave in self:
            if not leave.date_from:
                continue
            date_to_tz = user_tz.localize(leave.date_from) + relativedelta(
