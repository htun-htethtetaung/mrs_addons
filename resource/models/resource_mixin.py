...
class ResourceMixin(models.AbstractModel):
    ...
    def _get_work_days_data_batch(
        ...
        for record in self:
            mapped_resources[
                calendar or record._get_calendar(from_datetime)
            ] |= record.resource_id
    ...
    def _get_leave_days_data_batch(
        ...
        for record in self:
            mapped_resources[
                calendar or record.resource_calendar_id
            ] |= record.resource_id
    ...
    def list_work_time_per_day(
        ...
        intervals = calendar._work_intervals_batch(
            from_datetime, to_datetime, resource, domain, compute_leaves=compute_leaves
        )[resource.id]
