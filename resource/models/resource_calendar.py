...
class ResourceCalendar(models.Model):
    ...
    def default_get(self, fields):
        ...
        if "attendance_ids" in fields and not res.get("attendance_ids"):
            ...
            if (
                not company.resource_calendar_id.two_weeks_calendar
                and company_attendance_ids
            ):
                res["attendance_ids"] = [
                    (
                        0,
    ...
    def _compute_attendance_ids(self):
        for calendar in self.filtered(
            lambda c: not c._origin
            or c._origin.company_id != c.company_id
            and c.company_id
        ):
            ...
            calendar.update(
                {
                    ...
                    + [
                        (0, 0, attendance._copy_attendance_vals())
                        for attendance in company_calendar.attendance_ids
                        if not attendance.resource_id
                    ],
                }
            )
    ...
    def _compute_two_weeks_explanation(self):
        ...
        self.two_weeks_explanation = _(
            "The current week (from %s to %s) correspond to the  %s one.",
            first_day,
            last_day,
            week_type_str,
        )
    ...
    def _get_global_attendances(self):
        return self.attendance_ids.filtered(
            lambda attendance: attendance.day_period != "lunch"
            and not attendance.date_from
            and not attendance.date_to
            and not attendance.resource_id
    ...
    def _attendance_intervals_batch(
        ...
        attendance_per_resource = defaultdict(
            lambda: self.env["resource.calendar.attendance"]
        )
        ...
        for day in days:
            ...
            for attendance in attendances:
                ...
                if attendance.resource_id:
                    per_resource_result[attendance.resource_id].append(
                        (day_from, day_to, attendance)
                    )
                else:
            ...
            for resource in resources:
                if resource in per_resource_result:
                    ...
                    result_per_resource_id[resource.id] = Intervals(
                        itertools.chain(res, resource_specific_result)
                    )
                else:
    ...
    def _unavailable_intervals(
        self, start_dt, end_dt, resource=None, domain=None, tz=None
    ):
        if resource is None:
            resource = self.env["resource.resource"]
    ...
    def _get_attendance_intervals_days_data(self, attendance_intervals):
        ...
        return {
            ...
            "days": float_round(
                sum(day_days[day] for day in day_days), precision_rounding=0.01
            ),
    ...
    def _get_closest_work_time(
        self, dt, match_end=False, resource=None, search_range=None, compute_leaves=True
    ):
        ...
        if (
            not dt.tzinfo
            or search_range
            and not (search_range[0].tzinfo and search_range[1].tzinfo)
        ):
    ...
    def _get_unusual_days(self, start_dt, end_dt):
        ...
        works = {
            d[0].date() for d in self._work_intervals_batch(start_dt, end_dt)[False]
        }
        return {
    ...
    def _get_max_number_of_hours(self, start, end):
        ...
        for attendance in self.attendance_ids.filtered(
            ...
            and (
                (not a.date_from or not a.date_to)
                or (a.date_from <= end.date() and a.date_to >= start.date())
            )
        ):
            mapped_data[(attendance.week_type, attendance.dayofweek)] += (
